...
import {
  ...
  markSelectedIdentified, undoLastAction, setOnExcludeCallback,
} from "./map.js";
import {
  populateSelects, populateAMButtons, syncAMButtons,
  onFilterChange, onSearchChange, updateStats, updateLegend,
} from "./filters.js";
...
import { exportExcludedZips, exportIdentifiedZips, exportSelectedZips, exportFullMaster } from "./exports.js";
...
function initApp() {
  ...
  Promise.all([dataPromise, topoPromise, savedPromise]).then(function (results) {
    ...
    if (isBackendMode()) {
      ...
      document.getElementById("lastUpdated").textContent = "Data: April 2026";
    }
  }).catch(function (err) {
...
function setupEventListeners() {
  ...
  document.getElementById("exportFullMaster").addEventListener("click", function () {
    trackEvent("export_full_master");
    exportFullMaster();
  });
